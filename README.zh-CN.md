...
给小米 MiMo 这场 200 万美元以上的公开强化学习训练做的增强遥测层：可交互的指标解读、
跨曲线相关性分析，以及按训练步回放历史。

**这是什么。** 小米 MiMo 团队在 <https://mimo.xiaomi.com/rl/> 公开了 `mimo-v2.6-pro` 和
`mimo-v2.6-flash` 两次强化学习训练的实时看板。本项目把这条数据流存档、回放并讲清楚：
把公开 JSON 接口增量同步到本地仓库，在原始页面上加一层带历史回放、指标解读、数据洞察、
溯源与跨图分析的遥测层，并配上一套中英双语的成篇分析。
...
上游看板只能看**"现在"**。训练跑到第三天，你就再也看不到：

- 曲线早些时候是什么形状——只显示最新一步；
- 某一步完成时公告说了什么、那一刻评测跑到第几步；
- 一个指标到底在量什么——官方只给一句英文说明，而且只有 22 个指标有说明。

本项目把这几件事补上：把站点发布过的每个数值存进一份权威的本地仓库，在原页面上加一层回放，
并给每条指标、每条结论配上一篇解读、一条来源，或者一条能自己复算的脚本。
...
仓库自带一份完整数据快照（`data/store/`，pro 到第 27 步、flash 到第 30 步），
...
是由它派生的索引，删掉随时能用 `bun run reindex` 重建。`src/server.ts` 提供看板：在线时转发几个
小的实时接口，大的序列接口走本地仓库；回放模式下所有数据都从仓库重建。`src/export.ts` 读同一份仓库，
产出离线包。
