...
[![mimo-v2.6 RL telemetry —— 归档 60 步、每步 2000+ 指标、116 条指标解读](site/og-card.png)](https://huanghw1989.github.io/mimo-rl-telemetry/)

给小米 MiMo 这两场强化学习训练做的增强遥测层 —— **两个 run 都已跑到第 30 步收尾，合计 347 万美元**：
可交互的指标解读、跨曲线相关性分析，以及按训练步回放历史。

**一条链接能带到哪。** URL 里装的是**读数状态**，不只是一个页面。已经收尾的 `mimo-v2.6-pro`
在 DeepSWE v1.1 上连跌三个评测点（70.92 → 70.36 → 69.35 → 68.56），随后跳到整轮最大值 72.57。
打开下面这条就已经停在那一步了 —— 不用从约 2000 条曲线里翻找，也不用自己点到位：

| 链接 | 打开什么 |
| --- | --- |
| [`#step/pro/26/deepswe`](https://huanghw1989.github.io/mimo-rl-telemetry/#step/pro/26/deepswe) | 联动已开启、第 26 步已锁定，全页图表都按第 26 步读数，DeepSWE 那张卡片滚到眼前并描边标出 |
| [`#step/pro/25/deepswe`](https://huanghw1989.github.io/mimo-rl-telemetry/#step/pro/25/deepswe) | 同上，往前一步 —— 跳升前的最后一个点 |
| [`#corr/pro/26/deepswe`](https://huanghw1989.github.io/mimo-rl-telemetry/#corr/pro/26/deepswe) | **分析这一步**面板已经打开：第 26 步哪些指标异动，锚点就是 DeepSWE |

这些是浏览器就地解析的 hash 路由，所以在线站、导出的离线副本、以及回放切面（`?asof=<epoch>`）
里都是同一套行为。
...
**两个 run 停在哪。** 训练都已结束，所以这份看板现在当作一份"已完成的记录"来读，而不是实时页：

| Run | 步数 | 时间窗（北京时间） | 挂钟 | 花费 | 重启 |
| --- | --- | --- | --- | --- | --- |
| `mimo-v2.6-pro` | 30 | 2026-09-15 18:32 → 2026-09-21 02:01 | 5 天 7 小时 29 分 | **$2,620,671** | 14 |
| `mimo-v2.6-flash` | 30 | 2026-09-15 23:16 → 2026-09-19 10:22 | 3 天 11 小时 06 分 | **$854,045** | 5 |
| **合计** | **60** | — | — | **$3,474,716** | 19 |

归档的评测点**不是整齐地落后**，而是三个榜各停在各处：DeepSWE 两个 run 都到第 30 步，
In-house Coding 与 AutomationBench 的 pro 停在第 29 步（flash 三个榜都已跑满）。
不存在「对每个榜都是最新」的那一步，所以评测数字只有**连同榜名和步号**一起说才有意义。
两次 run 的事故也都是公开的：pro 在第 17 步因 MoE 专家负载不均导致 GPU OOM 重启，
之后团队又过滤掉了对当前 pro 已经过易的题目。
...
| 跨图读数 | 每张图各管各的 | 图表联动：在任意一张图上点某一步，所有图一起跳过去，地址栏同时记住这一步（[示例](https://huanghw1989.github.io/mimo-rl-telemetry/#step/pro/26/deepswe)） |
| "这一步为什么跳" | 无 | **分析这一步**：全指标异常度、与锚点曲线的秩相关与去趋势相关，以及这一步时段内的重启 / 版本切换 / 公告（[示例](https://huanghw1989.github.io/mimo-rl-telemetry/#corr/pro/26/deepswe)） |
...
仓库自带一份完整数据快照（`data/store/`，pro 到第 30 步、flash 到第 30 步，两个 run 都已跑完），
...
联动期间读数框不会自己消失，所以底色画成半透明 —— 它正要说明的那段曲线仍然看得见。
每锁一次步都会写进地址栏，看到的那个状态就是能发给别人的那个状态。

### 深链

地址栏装的是读数状态，所以链接可以指向某一步，而不只是某个页面：

| 链接 | 落在哪 |
| --- | --- |
| `#step/<run>/<step>` | 联动打开、该步锁定，每张图都按这一步读数 |
| `#step/<run>/<step>/<anchor>` | 同上，并把锚点那张图滚到眼前、描边标出 |
| `#corr/<run>/<step>[/<anchor>]` | 同上，且**分析这一步**已经打开 |
| `?asof=<epoch>` | 某个历史回放切面；可与上面任意一条叠加 |
| `#chart/<tag>` | 单个指标的整屏图表 |
| `#doc/metrics/@<tag>` | 某条解读，打开并滚到那一条 |
| `#metrics/<path>` | 某个指标分组 |

`<run>` 是 `pro` 或 `flash`。`<anchor>` 就是你点住的那条曲线：评测榜写榜单代号
（`deepswe`、`inhouse-coding`、`automation`），训练指标写 URL 编码后的指标名
（`actor%2Fentropy_loss`、`dynsam%2Favg@n`）。链接不需要任何预先算好的东西 ——
页面在加载时读 hash，所以离线副本认同一套 URL。
...
两边数字完全一致 —— 仓库里有一个 80 组 run/步/锚点的一致性校验脚本守着这句话。面板自己也有 URL
（`#corr/<run>/<step>/<anchor>`），所以单个结论可以直接给链接，而不是用文字描述。
...
下面每一篇都是**带自己数据切面**的独立分析，写在两个 run 还在推进的时候。所以「一句话结论」
引用的是该篇切面下的数字，可能与最终切面（pro 与 flash 都已停在第 30 步）不一致；
篇内会写明用的是哪个切面。要更新就重跑该篇自己的命令。两处最常被断章取义的最终切面值：
步耗时缺口 pro **32.13 h**（$660,548，账单的 25.2%）、flash **14.15 h**（$145,403，17.0%）；
DeepSWE 单步最小可测变化 pro **4.52**、flash **5.90**。
...
下面这些数字每条都由[「每个数字都能复算」](#每个数字都能复算)里的脚本从 `data/store/` 现算。
第 1、3~7 条取自它们写作时的分析窗口（pro 第 24 步 / flash 第 30 步），是一个冻结的窗口而不是最终状态；
第 2、4 条已经更新到训练收尾时的口径（两个 run 都到第 30 步，见上面「两个 run 停在哪」）。
...
2. **`timing_s/step` 和运行卡片上的"本步已用时"都不含重启等待时间。** 跑完全程后，pro 少报了
   **32.1 小时**（占总账单 25.2%），折合约 **66.1 万美元**；flash 少报 14.1 小时（占 17.0%），
   约 **14.5 万美元**。没有重启的步上报值和真实值差 10 秒以内，有重启的差几小时——
   所以这个缺口确实来自重启。缺口不等于纯停机：其中还含每次重启固定 12~13 分钟（pro）/
   5~6 分钟（flash）的恢复开销，以及跑完又被 `redo` 作废的重算。
...
4. **成本的核心差别在"同一个 token 要多少张卡"，不在"做了多少事"。** 两边单步的训练 token 量
   差不多（pro 平均 **25.0 亿**、flash **27.1 亿**），但每 **10 亿 token** 的成本 pro 是
   **3.49 万美元**、flash 是 **1.05 万美元**，差 **3.3 倍**。最终累计：
   pro **262.1 万美元** / flash **85.4 万美元**。
